...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      let activeChallenge =
        await ActiveChallegeService.getSingleActiveChallenge(req.params.userId);
      console.log("hi");
...
router.get("/start/:activeChallengeId", async (req: Request, res: Response) => {
  try {
    let activeChallenge: GetActiveChallToStartReq | null =
      await ActiveChallegeService.getActiveChallengeToStartScreen(
        req.params.activeChallengeId
      );
    res.send(activeChallenge);
  } catch (error) {
    res.status(400).send(error);
...
router.get(
  "/status/:activeChallengeId",
  async (req: Request, res: Response) => {
    try {
      let userId = req.body.userId;
      let startDeilyDeck: GetStatusDoneCardsRes =
        await ActiveChallegeService.getStartDailyDeck(
          userId,
          req.params.activeChallengeId
        );
...
router.post("/", async (req: Request, res: Response) => {
  try {
    let activeChallenge = await ActiveChallegeService.createNewActiveChallenge(
      req.body
    );
...
// תשובה על קלף ספציפי
router.post(
  "/:activeChallengeId/card/:cardId",
  async (req: Request, res: Response) => {
    try {
      let activeChallengeId = req.params.activeChallengeId;
      let cardId = req.params.cardId;
      await ActiveChallegeService.handleCardAnswer(
        activeChallengeId,
        cardId,
        req.body
      );
...
router.post("/join/:challengeId", async (req: Request, res: Response) => {
  try {
    ...
    const result = await ActiveChallegeService.joinActiveChallenge(
      userId,
      activeChallengeId
    );
    res.send(result);
