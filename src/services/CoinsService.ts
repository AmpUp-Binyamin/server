...
export default class CoinsService {
    ...
    static async addCoins(data: CoinsRequest): Promise<object> {
        let activeChallenge: IActiveChallenge | null = (await this.activeChallengeController.readOne(data.activeChallengeId))
        if (!activeChallenge) throw 'activeChallenge is not found'
        let currentDay = await this.getCurrentChallengeDay(activeChallenge)
        let cards: IActiveCard[] | undefined = activeChallenge.cards.filter(c =>
            String(c.member) == String(data.userId) && c.challengeDay == currentDay
        )

        let cardsOfChallenge = (await this.challengeController.readOne(String(activeChallenge.challenge)))?.cards
        .filter((c) => c.day == currentDay && c.cardType != 'support' && c.cardType != 'share' && c.cardType != 'lottery')
