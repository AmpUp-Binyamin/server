...
export default class MemberService {
  ...
  static async getMemberChallenges(id: string): Promise<any> {
    let member1 = await this.controller.readPopulate(
      id,
      [
        {
          path: "myActiveChallenge",
          select: "startDate endDate",
          populate: {
            path: "challenge",
            populate: {
              path: "creator",
              select: "fullName img picture",
            },
            select: "challengeName",
          },
        },
      ],
      "myActiveChallenge myChallenge"
    );

    const myChallenges = (member1?.myActiveChallenge as IActiveChallenge[]) || [];
    return myChallenges.map((c) => ({ ...c, startDate: '04-09', endDate: '11-09' }));
  }
}
