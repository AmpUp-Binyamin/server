...
export default class CoinsService {
  ...
  static async addCoins(data: CoinsRequest): Promise<object> {
    let activeChallenge: IActiveChallenge = (
      await this.activeChallengeController.read({ challenge: data.challengeId })
    )[0];
    ...
    let cardsOfChallenge = (
      await this.challengeController.readOne(data.challengeId)
    )?.cards.filter(
      (c) =>
        c.day == currentDay &&
        c.cardType != "support" &&
