...
const validation: ValidationRules = {
  fullName: (fullName: string) => {
    if (fullName.length <= 0) throw 'the name is not exist';
  },
  email: (email: string) => {
    if (!regexEmail.test(email)) throw 'the email is not valid';
  },
  phone: (phone: number) => {
    if (!regexPhone.test(String(phone))) throw 'the phone is not valid';
  },
  link: (link: string) => {
    if (!regexLink.test(link)) throw 'the link is not valid';
  },
  linkToSocialNetworks: (linkToSocialNetworks: string[]) => {
    linkToSocialNetworks.forEach((l) => {
      if (!regexLink.test(l)) throw 'there is link in list links is not valid';
    });
  },
};
...
export default class MemberService {
  ...
  static async addNewStoreItem(
    memberId: string,
    storeItem: IMemberItem,
  ): Promise<IMember | null> {
    return await this.controller.updateStoreItem(memberId, storeItem);
  }
  static async updateMemberCoins(
    memberId: string,
    newCoins: IMyCoins[],
  ): Promise<IMember | null> {
  ...
  static async getPersonalInfo(id: string): Promise<IMember | null> {
    const memberInfo: IMember | null = await this.controller.readOneProj(
      id,
      '-coins -notifications -_id',
    );
    return memberInfo;
  }
  ...
  static async findNewMemberInvites(
    createdMemberId: string | ObjectId,
  ): Promise<void> {
    const newMember = await this.controller.readOne(createdMemberId);
    if (newMember) {
      const foundChallengesId = await this.activeChallengeController.readSelect(
        { invited: newMember.email },
        '_id',
      );
  ...
  static async updateMember(
    ...
    (Object.keys(validation) as (keyof UpdateMemberRequest)[]).forEach(
      (key) => {
        if (data[key]) {
