...
export default class CardService {
  ...
  static async crateCard(data: CreateCardRequest) {
    //: Promise<IChallenge>{

    const challenge = await this.controller.readOne(data.challengeId);
    if (!challenge) throw { code: 400, message: 'not found challenge' };
    if (String(data.userId) != String(challenge.creator))
      throw { code: 400, message: 'not creator' };
    ...
    if (existingCard) {
      throw {
        code: 400,
        message: `A card of type ${data.cardType} already exists for day ${data.day}`,
      };
    }

    if (card.cardType === 'media' && !card.media)
      throw { code: 400, message: 'type card media bat not found media' };
    const newCard = await this.controller.update(data.challengeId, {
      $push: { cards: card },
    });
    return newCard;
  }
  ...
  static async updateCard(data: CreateCardRequest) {
    ...
    let oldCard = challenge.cards.find(
    ...
    if (updatedCard.cardType === 'media' && !updatedCard.media)
      throw { code: 400, message: 'type card media bat not found media' };
