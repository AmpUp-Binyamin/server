...
export default class MemberController implements IController<IMember> {
  ...
  async readWithChallenge(id: string): Promise<IMember | null> {
    return await MemberModel.findById(id)
      .populate({
        path: "myChallenge",
        select:
          "challengeName creator duration cards.title cards.day cards.image cards.cardType  cards._id",
  ...
  async readStartDate(id: string): Promise<IActiveChallenge | null> {
    const activeChallenge = await ActiveChallengeModel.findOne(
      { challenge: id },
      "startDate cards"
    );
    if (!activeChallenge) {
