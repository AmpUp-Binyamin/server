...
async function go() {
  ...
  const m1 = await UserModel.create(users[0]);
  const m2 = await UserModel.create(users[1]);
  const m3 = await UserModel.create(users[2]);
  const m4 = await UserModel.create(users[3]);

  const members = [m1, m2, m3, m4]

  //await ChallengeModel.create>>
  let exampleChallenge = {
  ...
  members.forEach(async m => {
    m.notifications.push({
      "challenge": exampleChallenge._id,
      "type": "sent support",
      "title": "Sent Support",
      "content": `${m.fullName} sent support for the sixth challenge`,
      "sender": m.fullName
    })
    m.save()
  })
